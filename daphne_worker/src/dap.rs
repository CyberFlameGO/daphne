        ...
        match req.sender_auth {
            ...
            Some(DaphneWorkerAuth::CfTlsClientAuth {
                ...
                if let Some(ref taskprov_config) = self.config().taskprov {
                    ...
                    let (valid_cert_issuer, valid_cert_subjects) = if matches!(
                        sender,
                        Some(DapSender::Leader)
                    ) {
                        match taskprov_config.leader_auth {
                    ...
                    } else if matches!(sender, Some(DapSender::Collector)) {
                        match taskprov_config.collector_auth {
                        ...
                        warn!("request denied from unrecognized sender");
                        return Ok(false);
                    };
